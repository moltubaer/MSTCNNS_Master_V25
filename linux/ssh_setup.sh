...
# Initial updates and installation of packages
...
sudo sed -i 's/^#\?\(UsePAM\s*\).*$/\1no/' /etc/ssh/sshd_config
...
echo "Password authentication is disabled. Only key-based authentication is allowed."
