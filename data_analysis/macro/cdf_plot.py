...
def generate_plot(core_dir, op_dir, csv_files, output_root):
    ...
    plt.title(f"{core_title} - {op_title}", fontsize=15)
    ...
    out_base = f"{core}_{operation}_cdf".lower()
...
def main(input_root, output_root):
    for core_name in os.listdir(input_root):
        ...
        for op_name in os.listdir(core_path):
            ...
            if csv_files:
                generate_plot(core_path, op_path, csv_files, output_root)
